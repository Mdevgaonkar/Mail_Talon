    ...
    if (accessToken) {
        ...
        if (sheet_list.body.sheets.indexOf(current_sheet_name) == -1) {
            ...
            if (new_sheet.body) {
                ...
                let address = 'A1:L2';
                let new_table = await sheets.createNewTable(accessToken, excel_drive_item_id, current_sheet_name, current_table_name, address);
                if (new_table.body){
                    console.log('new table created');    
                }else{
                    console.log(new_table);
                    parms.errors.push(utils.error(new_table.errors, "create new table failed"));
                    parms.debug.push({
                ...
                parms.errors.push(utils.error(new_sheet.errors, "create new table failed"));
                parms.debug.push({
